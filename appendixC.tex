...
derived shouldTimeout(ch, MI, stateNumber) = return boolres in
  let pID = processModelIDOf(ch) in
...
\begin{listing}[H]
\begin{minted}{lexer.py:CoreASMLexer -x}
rule StartProcessModel(processModelID,
                       additionalInitializationSubject,
                       additionalInitializationAgent) =
  let pID = processModelID in
  local PI in
    seq
...
\end{minted}
\caption{StartProcessModel}
\label{lst:asm:StartProcessModel}
\end{listing}
...
rule StartMacro(MI, currentStateNumber, mIDNew, MINew) = {
  let pID = processModelIDFor(self) in
  ...
  if (selectAgentsDecision(ch, MI, s) != undef) then {
      ...
      seq
        foreach agent in selectAgentsDecision(ch, MI, s) do
          if (resolvedProcessModelID = pID) then
            ...
            local newPI in
            seq
              newPI <- StartProcessModel(resolvedProcessModelID,
                                         resolvedSubjectID, agent)
            next
              add [resolvedprocessModelID, newPI,
      ...
      selectAgentsProcessModelID(ch, MI, s) := undef
    ...
    selectAgentsProcessModelID(ch, MI, s) := resolvedProcessModelID
